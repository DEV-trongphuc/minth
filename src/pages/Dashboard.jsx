                   ...
                   <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                     <div>
                       ...
                       {Number(item.qty) === 0 && Number(item.ml) === 0 ? (
                         ...
                         <span style={{ fontWeight: 700, color: 'var(--warning-dark)', background: 'var(--warning-bg)', padding: '0.2rem 0.5rem', borderRadius: '4px', fontSize: '0.8rem', whiteSpace: 'nowrap' }}>Còn {item.qty} {item.unit ? item.unit.charAt(0).toUpperCase() + item.unit.slice(1) : 'Sản phẩm'} {Number(item.ml) > 0 ? `- ${item.ml} ml` : ''}</span>
                       )}
                   </div>
                 ))
                }
